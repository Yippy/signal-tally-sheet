...
function displayAbout() {
  displayModalDiagram(SpreadsheetApp.openById(SIGNAL_TALLY_SHEET_SOURCE_REDIRECT_ID), SIGNAL_TALLY_REDIRECT_SOURCE_ABOUT_SHEET_NAME, "B1", "B2", "B3", "B4");
}

function displayMaintenance() {
  displayModalDiagram(SpreadsheetApp.openById(SIGNAL_TALLY_SHEET_SOURCE_REDIRECT_ID), SIGNAL_TALLY_REDIRECT_SOURCE_MAINTENANCE_SHEET_NAME, "B1", "B2", "B3", "B4");
}

function displayBackup() {
  displayModalDiagram(SpreadsheetApp.openById(SIGNAL_TALLY_SHEET_SOURCE_REDIRECT_ID), SIGNAL_TALLY_REDIRECT_SOURCE_BACKUP_SHEET_NAME, "B1", "B2", "B3", "B4");
}

function displayAutoImport() {
  displayModalDiagram(SpreadsheetApp.openById(SIGNAL_TALLY_SHEET_SOURCE_REDIRECT_ID), SIGNAL_TALLY_REDIRECT_SOURCE_AUTO_IMPORT_SHEET_NAME, "B1", "B2", "B3", "B4");
}

function displayHoYoLab() {
  displayModalDiagram(SpreadsheetApp.openById(SIGNAL_TALLY_SHEET_SOURCE_REDIRECT_ID), SIGNAL_TALLY_REDIRECT_SOURCE_HOYOLAB_SHEET_NAME, "B1", "B2", "B3", "B4");
}
...
function displayReadme() {
  ...
  if (sheetSource) {
    ...
    var sheetToRemove = SpreadsheetApp.getActive().getSheetByName(SIGNAL_TALLY_README_SHEET_NAME);
      if(sheetToRemove) {
    ...
    if (settingsSheet) {
      ...
      sheetREADMESource = sheetSource.getSheetByName(SIGNAL_TALLY_README_SHEET_NAME+"-"+languageFound);
    }
    if (sheetREADMESource) {
      // Found language
    } else {
      // Default
      sheetREADMESource = sheetSource.getSheetByName(SIGNAL_TALLY_README_SHEET_NAME);
    }

    sheetREADMESource.copyTo(SpreadsheetApp.getActiveSpreadsheet()).setName(SIGNAL_TALLY_README_SHEET_NAME);
    ...
    var sheetREADME = SpreadsheetApp.getActive().getSheetByName(SIGNAL_TALLY_README_SHEET_NAME);
